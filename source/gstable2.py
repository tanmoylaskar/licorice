from defcosmos import cosmology
applyIC = True
...
class Physparams(object):
    ...
    def __init__(self, k = 0, p = 2.5, z = 0, e_e = 0.5, e_b = 0.5, n_0 = 1, Astar = 1, E52 = 1, t_jet = 1, A_B = 0, zeta = 1.0):
        ...
        #self.dL28 = cosmocalc(self.z)['DL_cm'] / 1E28
    ...
    #def Lorentz_factor(self, rstype, tdec):
    ## This function calculates the Lorentz factor at the deceleration time / shock crossing time,
    ## given the parameters of the forward shock - energy, density, redshift, and tdec
    ## ISM calculations are based on the energy injection paper, Sari & Meszaros (2002)
    ## Wind calculations are from the RS in wind environments paper, Zou, Wu, and Dai (2005)        
        ## ISM case
        #if (self.k == 0):
            #G = 4.7*((1+self.z)**3. * (self.n_0)**(-1.) * self.E52 * tdec**(-3.))**(1./8)
        
        ## Wind case; note that Gamma(tdec) does not depend on tdec for wind environments
        #elif (self.k == 2): 
            #if (rstype == 'rrs'):
                #G = 25.8     * ((tdec/(16.7/86400)) * self.Astar * (1.+self.z)**(-1.) * self.E52**(-1.))**(-0.25)
            #if (rstype == 'nrs'):
                #G = 10**(1.5)* ((tdec/(29.0/86400)) * self.Astar * (1.+self.z)**(-1.) * self.E52**(-1.))**(-0.25)
        #else:
